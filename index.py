...
def handler(event, context):
    ...
    # ==========================
    # 3. DIRECT PASS via Whitelists
    # ==========================
    try:
        if core.check_user_whitelist(user_code):
            final_result = {
                "decision": "PASS",
                "primary_threat": "NONE",
                "risk_score": 0,
            ...
            core.log_decision_to_db(
                user_code, final_txn_id, final_result, features, source
            )
    ...
    try:
        if dest_address and core.check_address_whitelist(dest_address, chain):
            final_result = {
                "decision": "PASS",
                "primary_threat": "NONE",
                "risk_score": 0,
    ...
    # ==========================
    blacklist_result = core.check_blacklists(features)
    if blacklist_result:
        source = "RULE_ENGINE_BLACKLIST"
        core.log_decision_to_db(user_code, final_txn_id, blacklist_result, features, source)
    ...
    # ==========================
    # 5. Greylist → HOLD + AI (Phase-2)
    # ==========================
    grey_result = core.check_greylist(features)
    if grey_result:
        rule_source = "RULE_ENGINE_GREYLIST"
        core.log_decision_to_db(user_code, final_txn_id, grey_result, features, rule_source)
        ...
        core.send_lark_notification(rule_payload)

        ai_result = core.call_gemini_reasoning_rest(features)
        ai_source = "AI_AGENT_GREYLIST"
        core.log_decision_to_db(user_code, final_txn_id, ai_result, features, ai_source)
